  ...
  public DatasetSpecification createOrUpdate(final DatasetId datasetInstanceId, final DatasetTypeMeta typeMeta,
    ...
    try (DatasetClassLoaderProvider classLoaderProvider =
      ...
      final DatasetType type = ImpersonationUtils.doAs(ugi, () -> {
        LOG.info("---- Getting dataset type {}", typeMeta.getName());
        ...
        LOG.info("---- Got dataset type {}", typeMeta.getName());
        return type1;
      });

      DatasetSpecification spec = ImpersonationUtils.doAs(ugi, () -> {
        LOG.info("---- Configuring dataset {} of type {}", datasetInstanceId.getDataset(), typeMeta.getName());
        ...
        LOG.info("---- Configured dataset {} of type {}", datasetInstanceId.getDataset(), typeMeta.getName());
        ...
        try {
          if (existing != null) {
            ...
            LOG.info("---- Creating dataset {} of type {}", datasetInstanceId.getDataset(), typeMeta.getName());
            admin.create();
            LOG.info("---- Created dataset {} of type {}", datasetInstanceId.getDataset(), typeMeta.getName());
          }
        } finally {
      ...
      LOG.info("---- Writing metadata for dataset {}", datasetInstanceId.getDataset());
      writeSystemMetadata(datasetInstanceId, spec, props, typeMeta, type, context, existing != null, ugi);
      LOG.info("---- Wrote metadata for dataset {}", datasetInstanceId.getDataset());
      return spec;
    } catch (Exception e) {
